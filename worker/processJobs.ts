...
async function evaluateTranscriptWithGemini(transcription: string, surgeryName: string, additionalContext: string): Promise<GeminiEvaluationResult> {
    console.log(`Starting Gemini evaluation for ${surgeryName}...`);
    ...
    if (!EVALUATION_CONFIG) {
        throw new Error(`Invalid surgery name provided to Gemini evaluator: ${surgeryName}`);
    }
...
async function processJob(job: Job) {
    ...
    if (!job.gcsUrl || !job.surgeryName) {
      throw new Error('Job is-missing gcsUrl or surgeryName.');
    }
