...
// ... (Your interfaces and configs remain the same)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    console.log(`[CRON] Authorization Header: ${req.headers.authorization}`);
    
    if (req.method !== 'GET') {
        console.log(`[CRON] Blocked non-GET request with method: ${req.method}`);
        return res.status(405).json({ message: 'Method Not Allowed' });
    }

    if (req.headers.authorization !== `Bearer ${process.env.CRON_SECRET}`) {
        console.error('[CRON] Authorization failed. Incoming token does not match CRON_SECRET.');
        return res.status(401).json({ error: 'Unauthorized' });
    }

    console.log("[CRON] Authorization successful. Checking for a pending job.");
